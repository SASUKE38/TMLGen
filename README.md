...
The steps below describe how to link a generated .tml file to its dialog. The editor currently does not seem to support overriding timelines directly; these steps detail a method to overcome this restriction.
...
7. If the timeline has templates, perform the following to ensure that changes made to them are correctly reflected in your mod. Skip to step 8 otherwise.
    - A folder called Timeline Templates will be created in the same location as the TMLGen tool. A folder containing copies of the templates will be located there.
    - Rename the folder containing the templates to the timeline's GUID. This can be done by locating the timeline in the Resource Manager, right-clicking it, and selecting 'Copy GUID to Clipboard.' Note that if you override the timeline using the steps above that the GUID will change from the vanilla GUID.
    - Place the folder in your mod's Public\mod name\TimelineTemplates folder.
    - You might need to restart the editor for it to recognize your changes. You can check if the templates are correctly overriden by selecting one after you open the timeline and verifying that the path to its source file (found in the sidebar) contains your mod's name.
8. Locate the generated source and scene files in your mod's Public\mod name\Timeline\Generated folder.
9. Replace these files with the originals from the unpacked game data. If necessary, rename the originals so they have the same name as the files you replace.
10. Locate the .tml file generated by the editor in your mod's Editor\Mods\mod name\Timeline\Generated directory.
11. Replace this file with the one generated from the TMLGen tool. Make sure it has the same name as the replaced .tml.
12. Return to the opened dialog.
13. Open the timeline.
